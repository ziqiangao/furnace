...
SafeWriter *DivEngine::saveText(bool separatePatterns)
{
  ...
  for (int i = 0; i < song.insLen; i++)
  {
    ...
    if (ins->ws.enabled)
      {
        w->writeText(fmt::sprintf("- Wavetable Synth parameters\n"));
        w->writeText(fmt::sprintf("  - Global: %s\n",trueFalse[ins->ws.global]));
        w->writeText(fmt::sprintf("  - Type: %i\n",ins->ws.effect));
        w->writeText(fmt::sprintf("  - Wave 1: %i\n",ins->ws.wave1));
        if (ins->ws.effect >= DIV_WS_NONE_DUAL) w->writeText(fmt::sprintf("  - Wave 2: %i\n",ins->ws.wave2));
        
        w->writeText(fmt::sprintf("  - Update Rate: %i\n",ins->ws.rateDivider));
        w->writeText(fmt::sprintf("  - Speed: %i\n",ins->ws.speed));

        w->writeText(fmt::sprintf("  - Amount: %i\n",ins->ws.param1));
        if (ins->ws.effect == DIV_WS_PHASE_MOD) w->writeText(fmt::sprintf("  - Power: %i\n",ins->ws.param2));
      }
